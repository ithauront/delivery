...
para isso vamos usar o terceiro parametro do useReducer que pode ser uma função que pode ser disparada assim q o reducer for criado para eu recuperar os dados iniciais do meu reducer de algum outro lugar. o useReducer fica assim
  ...
  o erro do history era concertado na proxima aula.
  o erro foi o seguinte
  por qua o motodo toDate que é chamado em varios lugares.
  o erro ta la no history index.
  isso estava acontecendo porque o local storage so suporta strings. e o date não é isso entao essa data
       {formatDistanceToNow(cycles.startDate, {
                      addSuffix: true,
                      locale: ptBR,
                    })}
                    essa parte do codigo então precisa serenvelopada em um new Date
...
o que issovai fazer é que se o valor que a gente passar no cycles.starDate for uma data não vai fazer nada e se for um string ela vai converter em uma data.